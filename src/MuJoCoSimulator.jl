...
export mjSim, Simulator, simulate
...
function sensorupdate(s::mjSim)
    ...
    for n = 1:m.m[].nsensor
        ...
        # update linepnt
        mj.set(s.figsensor, :linepnt, min(Int(mj.MAXLINEPNT) - 1, p + 2dim), lineid)
    end
end
...
function mouse_move(s::mjSim, window::GLFW.Window, xpos::Float64, ypos::Float64)
    ...
    mod_shift = GLFW.GetKey(window, GLFW.KEY_LEFT_SHIFT) ||
    ...
    if s.pert[].active != 0
        mjv_movePerturb(s.m.m, s.d.d, action, dx / height, dy / height, s.scn, s.pert)
    else
        mjv_moveCamera(s.m.m, action, dx / height, dy / height, s.scn, s.cam)
    end
end
...
function drop(window::GLFW.Window, count::Int, paths::String) end
...
function render(s::MuJoCoSimulator.mjSim, w::GLFW.Window)
    wi, hi = GLFW.GetFramebufferSize(w)
...
function loadmodel(modelfile, width, height)
    ...
    mj_forward(m, d)
    s = MuJoCoSimulator.start(m, d, width, height)
